...
class DelightCard {
  ...
  factory DelightCard.fromJson(Map<String, dynamic> json) => DelightCard(
    delightId: json['delight_id']?.toString() ?? json['id']?.toString() ?? '',
    content: Recommendation.fromJson(
      json['content'] is Map ? Map<String, dynamic>.from(json['content']) : const {},
    ),
    reason: AppUtils.decodeHtml(json['reason']?.toString() ?? ''),
    bridgeLogic: AppUtils.decodeHtml(json['bridge_logic']?.toString() ?? ''),
    speculationDomain: json['speculation_domain']?.toString() ?? '',
    speculationConfidence: (json['speculation_confidence'] as num?)?.toDouble() ?? 0,
...
class RuntimeStatus {
  ...
  factory RuntimeStatus.fromJson(Map<String, dynamic> json) => RuntimeStatus(
    ...
    activeSources:
        (json['active_sources'] as List?)?.map((e) => e.toString()).toList() ?? const [],
    sourceStatus:
        json['source_status'] is Map ? Map<String, dynamic>.from(json['source_status']) : const {},
    pendingNotifications: (json['pending_notifications'] as num?)?.toInt() ?? 0,
